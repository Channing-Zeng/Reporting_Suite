...
print join("\t", qw(Sample Chr Start ID Ref Alt Type Effect Functional_Class Codon_Change Amino_Acid_Change Amino_Acid_Length Gene Transcript_bioType Gene_Coding Transcript Exon COSMIC_CDS_Change COSMIC_AA_Change End Depth AlleleFreq Bias Pmean Pstd Qual Qstd SBF GMAF VD CLNSIG ODDRATIO HIAF MQ SN AdjAF Shift3 MSI dbSNPBuildID N_samples N_Var Pcnt_sample Ave_AF PASS Var_Class)), "\n";
my @data;
my %sample;
my %var;
...
foreach my $d (@data) {
    ...
    $pass = "NonClnSNPle" if ( checkCLNSIG($d->[30]) == -1 && $class ne "COSMIC" );
