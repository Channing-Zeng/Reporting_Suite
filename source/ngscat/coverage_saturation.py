...
def coverage_saturation_local(bamlist, targets, depthlist, coverage, legend, fileout, executiongranted=None,
                              status=None, slopes=None, tmpdir=None, warnthreshold=1e-5):
    """************************************************************************************************************************************************************
    ...
    for i, bam in enumerate(bamlist):
        for depth in depthlist:
            ...
            run("""qsub -N """ + jobid + """ -o """ + TMP + """/logs/ -e """ + TMP + """/logs/ -l h_vmem=3.0G -M fjavier@bioinfomgp.org -m beas -R y \
                    do_simulated_depth.sh """ + bam + """ """ + targets[i] + """ """ + str(depth) + """ """ + str(
                coverage) + """ """ + TMP + jobid, submit)
...
def main():
    ...
    ************************************************************************************************************************************************************   
    ...
    parser.add_option("--targets", dest="targets",
                      help="""String containing comma separated names of the beds containing the targets for each run.""")
    parser.add_option("--depthlist", dest="depthlist",
                      help="""Comma separated integers containing the run depths to test (millions of reads).""")
    parser.add_option("--coverage", dest="coverage",
                      help="""Integer containing the coverage threshold per position to test.""")
    parser.add_option("--legend", dest="legend",
                      help="""String containing a comma separated list of descriptions describing each of the files that will be processed. These descriptions will form the legend of the bar plot.""")
    parser.add_option("--fileout", dest="fileout",
                      help="""String containing the name of the file where the graph will be saved.""")
    parser.add_option("--multithread", dest="multithread",
                      help="""{y,n} to indicate whether to run as jobs in SGE queue or as threads in the local machine.""")
    parser.add_option("--submit", dest="submit",
                      help="""Optional. {y,n} to indicate whether jobs should be submitted or not. Default = y""",
                      default='y')
    ...
    if (options.multithread == 'y'):
        # call core function
        coverage_saturation_local(options.bamlist.split(','), options.targets.split(','),
                                  map(string.atoi, options.depthlist.split(',')), string.atoi(options.coverage),
                                  options.legend.split(','), options.fileout)
    else:
        # call core function
        coverage_saturation(options.bamlist.split(','), options.targets.split(','),
                            map(string.atoi, options.depthlist.split(',')), string.atoi(options.coverage),
                            options.legend.split(','), options.fileout,
                            options.submit == 'y')
