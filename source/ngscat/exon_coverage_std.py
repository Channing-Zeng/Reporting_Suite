...
def load_coverage_file_per_exon(filename):
    ...
    *********************************************************************************************************************************************************"""
    ...
    for i, line in enumerate(fd):
        ...
        # Only exons longer than 3 nts
        if (exon_length > 3):
            if (curr_exon <> (parts[0] + "-" + parts[1] + "-" + parts[
                2])):  # New exon found
                curr_exon = parts[0] + "-" + parts[1] + "-" + parts[2]
                next_exon_idx += 1
                if (next_exon_idx >= 2):  #Measure normalized standard deviation values
                    coverage = numpy.array(coverage)
                    if (
                    len((coverage > 0).nonzero()[0])):  # Avoid exons with zero coverage values in all bases of exons
...
def exon_coverage_std_lite(groups, fileoutprefix, legend=None, executiongranted=None, status=None, coveragestd=None,
                           warnthreshold=0.3):
    """************************************************************************************************************************************************************
        JPFLORIDO
    ...
    ws.write(0, 5, 'Minimum', header_style);
    ...
    for i, std_sampling in enumerate(global_stdsampling):
        ...
        if (status <> None):
            status.value = status.value and bool(mean <= warnthreshold)
...
def exon_coverage_std(groups, fileoutprefix, bedfilename, legend=None, normalize=True):
    ...
    ************************************************************************************************************************************************************"""
    ...
    # add some
    fig.suptitle('Distribution of coverage standard deviations (normalized) across exons', fontsize=14,
                 fontweight='bold')
    ax.set_ylabel('Frequency')
    ax.set_xlabel('Normalized standard deviation')
    ax.set_xlim(0, 1)

    boxplot.suptitle('Distribution of coverage standard deviations (normalized) across exons', fontsize=14,
                     fontweight='bold')
    axb.boxplot(global_stdsampling)
...
def main():
    ...
    ************************************************************************************************************************************************************
    ...
    parser.add_option("--groups", dest="groups",
                      help="""String containing a comma separated list of files, each containing the list of bam files for a given group.""")
    ...
    parser.add_option("--bed", dest="bed",
                      help="""String containing the file name of the bed containing the regions to analyze.""")
    ...
    parser.add_option("--normalize", dest="normalize",
                      help="""Optional. {y,n} to indicate whether bam files should be normalized or not.""",
                      default='y')
    ...
    # call core function
    exon_coverage_std([group.split(',') for group in options.groups.split('-')], options.fileoutprefix, options.bed,
                      options.graphlegend.split(','), options.normalize == 'y')
