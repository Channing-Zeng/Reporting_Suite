...
class BamFile(pysam.Samfile):
    ...
    def draw_coverage_distribution(self, on_points, noton_points, off_points, base0, lengths, fileout):
        ...
        rects.append(
            ax.plot([point[0] for point in noton_points], [point[1] for point in noton_points], color="#00ff00"))
        ...
        *******************************************************************************************************************************************"""
        ...
        for currentChromosome in finalBed.chrs.keys():
            ...
            if (currentChromosome in sortedBam.references):
                if (sortedBam.count(str(
                        currentChromosome)) > 0):  # There might me information about the chromosome in the BAM header but no reads
                    ...
                    numpy.random.seed(1)
                    selected = numpy.random.uniform(size=len(initPositions)) <= (
                    numReadsDesired * 1.0 / sortedBam.nreads())
                    ...
                    while indexEnd < totalPositions_end:  # While there are reads to be visited
                        if (indexInit < totalPositions_init):  # There are still reads that have to be visited
                            if (selectedPositions_init[indexInit] < selectedPositions_end[
                    ...
                    if (bedGraphFile != None):
                        ...
                        fdw_bedGraph.write('track type=bedGraph name="coverage_chr_' + str(
                            currentChromosome) + self.filename + '" description="coverage per position for ' + self.filename + ' chromosome ' + str(
                            currentChromosome) + '"\n')
                        ...
                        for index in range(0, len(positionArray_bedGraph) - 1):
                            if (coverageArray_bedGraph[index] != 0):
                                fdw_bedGraph.write(
                                    str(currentChromosome) + ' ' + str(positionArray_bedGraph[index]) + ' ' + str(
                                        positionArray_bedGraph[index + 1]) + ' ' + str(
                                        coverageArray_bedGraph[index]) + '\n')
            ...
            firstPosition = len(
                positionArray_intersect)  # First position in the intersection vectors (positionArray_intersect and coverageArray_intersect)
            ...
            for currentExon in finalBed.chrs[str(currentChromosome)]:
                ...
                if (indexCoverage > 0 and indexCoverage < numPositions and positionArray[
                    nextPositionChange] > exon_init):
                ...
                if (
                        endPosition != startPosition or thereAreReads):  # Puede darse el caso de que un exon este en una sola coordenada de positionArray -> tenerlo en cuenta!!!!
                    ...
                    if (writeToFile == None):
                        ...
                        for currentKey in sorted(dicExon.keys()):
                            if (dicExon[
                                    currentKey] == -1):  # Take into account the coverage of the last position (key) that has coverage
    ...
    def myReadsOnTarget(self, target):
        ...
        *******************************************************************************************************************************************"""
        ...
        for currentRead in self:
            ...
            if (currentChromosome != previousChromosome):
                ...
                if (thereisInfo):  # A new chromosome is started. Store information about previous chromosome
                    ...
                    dicOnTarget[
                        str(previousChromosome)] = readsOnTargetChr  # Count reads on target for the current chromosome
                    dicOffTarget[str(
                        previousChromosome)] = readsOffTargetChr  # Count reads off target for the current chromosome
                    dicTotalReads[str(
                        previousChromosome)] = readsOnTargetChr + readsOffTargetChr  # Count all reads for the current chromosome
                ...
                if (
                    currentChromosome in finalBed.chrs.keys()):  # Check whether chromosome related to the read is in the BED file
                    targets_chr = numpy.array(finalBed.chrs[str(
                        currentChromosome)])  # all rows, 1st column -> exon_init positions; all rows, 2nd column -> exon_end positions
            ...
            if (
                currentChromosome in finalBed.chrs.keys()):  # Go through the process only if currentchromosome is present in the region
                thereisInfo = True
                ...
                while ((not stop) and currentPos < numRegions):
                    if ((currentReadInit >= targets_chr[currentPos, 0] and currentReadInit <= targets_chr[
                        currentPos, 1]) or
                            (currentReadEnd >= targets_chr[currentPos, 0] and currentReadEnd <= targets_chr[
                                currentPos, 1])):  # Read on target: read init inside the region or read end inside the region
                        ...
                        if (currentReadEnd < targets_chr[
                            currentPos, 0]):  # A read ends before the current region -> stop searching. Read off target
                ...
                if (
                    currentPos == numRegions):  # Read off target -> we moved along all regions and none of them mathes with the read
                    if (dicOffTargetChr.has_key((currentReadInit, currentReadEnd))):
                        dicOffTargetChr[(currentReadInit, currentReadEnd)] += 1
                    else:
                        dicOffTargetChr[(currentReadInit, currentReadEnd)] = 1
        ...
        for currentChromosome in finalBed.chrs.keys():
            if (
            not currentChromosome in dicOnTarget.keys()):  # If chromosome is not in the dictionary is because there are no reads for that chromosome
    ...
    def generate_ontarget_results(self, bamlist, nread, tread, onperchr, totalperchr, enrichment, percontarget, outdir,
        ...
        ************************************************************************************************************************************************************"""
        ...
        colours = ['#46a246', '#ff0000', '#00ff00', '#0000ff', '#cc0011', '#007722', '#110066', '#c1c1c1', '#544db1',
                   '#aa5198', '#bbd1e9', '#f1c4ab', '#24687a']
    ...
    def generate_duplicates_results(self, bamlist, ontargetreads, offtargetreads, onduplicates, offduplicates, outdir,
                                    legend):
        """************************************************************************************************************************************************************
        ...
        ************************************************************************************************************************************************************"""
        ...
        for j, bam in enumerate(bamlist):
            ...
            box = ax.get_position()
            ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
            ax.legend(tuple([rect[0] for rect in rects]), ('On target', 'Off target'), loc="upper left",
                      bbox_to_anchor=(1, 1))
        ...
        ************************************************************************************************************************************************************"""
        ...
        for i in range(len(bamlist)):
            ...
            if (tread[i] == nread[i]):
                ...
                enrichment[i] = (nread[i] * 1000.0 / targetsize) / ((tread[i] - nread[i]) * 1000.0 / (
                self.mappingsize() - targetsize))  # The same target must had been used for all bam files

        if (percontarget == None):
            percontarget = [None for i in range(len(bamlist))]

        self.generate_ontarget_results(bamlist, nread, tread, onperchr, totalperchr, enrichment, percontarget, outdir,
                                       legend)
        self.generate_duplicates_results(bamlist, nread, [tread[i] - nread[i] for i in range(len(nread))], onduplicates,
                                         offduplicates, outdir, legend)
        ...
        if (duplicates_status <> None):
            ...
            while (i < len(onduplicates) and duplicates_status.value):
                duplicates_status.value = bool(sum(onduplicates[i][1:]) > sum(offduplicates[i][1:]))
                i += 1
    ...
    def getBCQ_regions(self, regions, phredValue):
        ...
        *******************************************************************************************************************************************"""
        ...
        for currentRegion in sorted(regions.keys()):
            ...
            allReads = self.fetch(str(currentRegion[0]), initRegion,
                                  endRegion + 1)  # Fetch no devuelve la read que comienza justo en endRegion, asi que sumo 1. Las ventanas estan ya en base 1
            ...
            for currentRead in allReads:
                ...
                if (len(
                        currentRead.cigar) > 1):  # There are deletions and/or insertions. Build base calling vector according to these deletions/insertions
                    newQB = []
                    previousIndex = 0
                ...
                currentReadEnd = currentReadInit + len(
                    newQB) - 1  # Adapt the length of the read according to their insertions and or deletions (if any)

                if (
                    currentReadInit < initRegion):  # Read starts before the region -> avoid all values before the beginning of the region
                    initPos = initRegion
                ...
                while (currentPos <= currentReadEnd):  # Move along all BCQ values from the beginning to the end
                    if (newQB[indexBCQ] != 'NUL'):  # Due to the deletions
                        if (currentPos in dicBCQ.keys()):
                            dicBCQ[currentPos].append(
                                ord(newQB[indexBCQ]) - phredValue)  # Base calling quality (based on Phred+33)
                        else:
            ...
            if (len(allBCQ) > 0):
                regions[currentRegion] = numpy.array(
                    allBCQ).mean()  # Finally, get the average value of BCQ for the whole region
    ...
    def drawcoveragefeatures(self, coverage, sampling, fileout):
        ...
        ************************************************************************************************************************************************************"""
        ...
        coverage_rect = axcoverage.plot(x, numpy.array([values[1] for values in coverage]), color='#ff683e',
                                        linewidth=5)
        ...
        ax.legend(tuple([nvars_rect[0], coverage_rect[0]]), tuple(['# probes', 'Coverage']), loc="upper center",
                  prop={'size': 30})
