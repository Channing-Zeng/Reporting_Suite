...
def measureGCbias(wholeChromosome, currentChromosome, bedFile):
    ...
    **"""
    ...
    for currentExon in bedFile.chrs[str(currentChromosome)]:
        ...
        currentSequence = wholeChromosome[(
        exon_init - 1):exon_end]  # The first component is at 0 index and the last component is at n-1 index
        ...
        gccontent[currentChromosome, exon_init, exon_end] = ((countsG + countsC) / float(
            len(currentSequence))) * 100.0  # Percentage of GC content

    return gccontent


def gcbias_lite(coveragefile, bedfilename, reference, fileout, graphtitle=None, executiongranted=None, status=None,
                bedTools=False):
    """************************************************************************************************************************************************************
    ...
    if (len(coverage) > 1):
        ...
        if not bedTools:  # Own method
            ...
            if biopython_error:
                ...
                for line in fastaFile:  # Read each line of the fasta file
                    if line.startswith('>'):  # New chromosome starts -> reading a new line until another '>' is found
                        ...
                        if storeSequence:  # a chromosome has been read run gc bias
                            ...
                            storeSequence = False
                        currentChromosome = re.split(' +', line)[
                        ...
                        if (
                            currentChromosome in chromosomes):  # If current chromosome read in the FASTA file is in the list of chromosomes in the BED file
                            storeSequence = True
                ...
                my_ref = reference
                with open(my_ref, 'r') as handle:
                    reference_records = SeqIO.parse(handle, 'fasta')
                    for record in reference_records:
                        if record.id in chromosomes:
                            currentGCcontent = measureGCbias(record.seq.tostring(), record.id, finalBed)
