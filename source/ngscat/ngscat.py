...
def main():
    ...
    ************************************************************************************************************************************************************
    ...
    parser.add_option("--tmp", dest="tmp", help="""Optional. String indicating the path to a temporary directory where temporary files will be created. Default=<output_dir>/""", default=None)
    parser.add_option("--nt", "--threads", dest="nthreads", help="""Optional. Integer indicating the number of concurrent threads to launch. Default=2.""", default=2)
    parser.add_option("--debug", dest="debug", action="store_true", help="""Optional. Use this option to disable removing of temporary files.""")
    ...
    if(check_parameters(options,parser)):
        ...
        if(options.depthlist=='auto'):
            # call core function
            ngscat(options.bams, options.bed, options.out, options.reference, options.saturation=='y', int(options.nthreads), extend, options.depthlist,
                   map(float, options.coveragethresholds.split(',')), options.feature, options.tmp)
        else:        
            # call core function
            ngscat(options.bams, options.bed, options.out, options.reference, options.saturation=='y', int(options.nthreads), extend, map(float, options.depthlist.split(',')),
                   map(float, options.coveragethresholds.split(',')), options.feature, options.tmp)
