...
def histo(alldata, xlab, ylab, outdir, legend=None):
    ...
    for i in range(len(alldata)):
        ...
        ax2.plot([numpy.average(med.get_xdata())], [numpy.average(alldata[i])], color='w', marker='*',
                 markeredgecolor='k')
    ...
    if (legend <> None and len(legend) > 1):
        ...
        ax.legend(tuple([histogram[0] for histogram in histograms]), tuple(legend), loc="lower left",
...
def histo_CV(infiles, outdir, legend=None, executiongranted=None, status=None, meancoverage=None, warnthreshold=40):
    ...
    ws.write(0, 0, 'File', header_style);
    ws.write(0, 1, '# bases coverage 0', header_style);
    ws.write(0, 2, 'Q1', header_style);
    ws.write(0, 3, 'Q2', header_style);
    ws.write(0, 4, 'Q3', header_style);
    ws.write(0, 5, 'Maximum', header_style);
    ws.write(0, 6, 'Minimum', header_style);
    ws.write(0, 7, 'Mean', header_style)
    ...
    for i, dist in enumerate(alldata):
        ...
        ws.write(i + 1, 1, len(ndist) - len(ndist.nonzero()));
        ws.write(i + 1, 2, p25);
        ws.write(i + 1, 3, p50);
        ws.write(i + 1, 4, p75);
        ws.write(i + 1, 5, maximum);
        ws.write(i + 1, 6, minimum);
        ws.write(i + 1, 7, mean);
