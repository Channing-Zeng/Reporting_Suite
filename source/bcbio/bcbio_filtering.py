...
from source.variants.filtering import run_vcf2txt_vardict2mut_for_samples, combine_vcfs, combine_results
...
def finish_filtering_for_bcbio(cnf, bcbio_structure, callers):
    ...
    if errory:
        ...
        for sample_name, caller_name, fpath in errory:
            if not fpath:
                err('  For ' + str(sample_name) + ', ' + caller_name + ' VCF cannot be found')
            else:
                err('  For ' + str(sample_name) + ', ' + caller_name + ' VCF ' + str(fpath) + ' cannot be read')
