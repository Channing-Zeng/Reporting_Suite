...
def run_copy_number(sample_mapped_reads, gene_depth):
    list_genes_info = _report_row_to_object(gene_depth)

    med_depth = median([rec.min_depth for rec in list_genes_info])
    norm_depths_by_seq_distr = _get_norm_depths_by_seq_distr(sample_mapped_reads, list_genes_info)
    ...
    _print_report(list_genes_info, norm2, norm3, norm_depths_by_gene, norm_depths_by_seq_distr)
...
def _get_norm_depths_by_seq_distr(mapped_reads_by_sample, record_by_sample):
...
def _print_report(list_genes_info, norm2, norm3, norm_depths_by_gene, norm_depths_by_seq_distr):
    for gene_info in list_genes_info:
...
def _report_row_to_object(gene_depth):
    gene_details = []
