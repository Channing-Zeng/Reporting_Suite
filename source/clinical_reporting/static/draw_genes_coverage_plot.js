$(function() {
    var fullData = readJsonFromElement($('#gene_plot_data_json'));
    ...
    if (fullData != null) {
        if (fullData.hasOwnProperty('mutations')) {
            ...
            var i = 0;
            for (var k = 0; k < fullData['hits'].length; k++) {
                var data = fullData['hits'][k];
                var key = data['key'];
                drawGeneCovPlot(data, fullData, $('#' + key + '_gene_plot_placeholder'), $('#' + key + '_gene_plot_legend_placeholder'));
                showTip.tipElementExists = false;
                $('#' + k + '_gene_plot_header').css({color: colors[i]});
                i += 1;
            }
        }
    }
...
function drawGeneCovPlot(data, fullData, placeholder_el, legend_placeholder_el) {
    ...
    var geneNames = fullData.gene_names;
    var transcriptNames = fullData.transcript_names;
    var strands = fullData.strands;
    var ticksX = fullData.ticks_x;
    var linesX = fullData.lines_x;
    ...
    if (!info.isInitialized) {
        ...
        var aveDepths = data.gene_ave_depths;
        var depthInThreshold = data.covs_in_thresh;
        var mutInfoByGene = data.mut_info_by_gene;
