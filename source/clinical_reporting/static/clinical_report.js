...
var table_short_clone = {};
var table_full_clone = {};
...
$(function() {
    ...
    for (var t = 0; t < tables_short.length; t++){
      ...
      table_short_clone[switch_id] = $(table_short).clone();
      table_full_clone[switch_id] = $(table_full).clone();
...
function extendClick(switch_id) {
    ...
    table_div.prepend(table_full_clone[table_id]);
    ...
    table_full_clone[table_id] = table_full_clone[table_id].clone();
}
...
function reduceClick(switch_id) {
  if (switch_id[0].id) switch_id = switch_id[0].id;
    ...
    table_div.prepend(table_short_clone[table_id]);
    var table_short = table_div.find('.table_short');
    if (table_short) {
      table_short.show();
      if (msieversion() == 0) {
    ...
    table_short_clone[table_id] = table_short_clone[table_id].clone();
}
