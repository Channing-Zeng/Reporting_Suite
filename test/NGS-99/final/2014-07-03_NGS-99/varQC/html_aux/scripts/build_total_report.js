...
(function() {
  ...
  reporting.buildTotalReport = function(report, columnOrder, date) {
    var hue, legend, lightness, metric_html, pos, rec, recNum, result, sampleLink, sampleName, sampleReport, step, table, value, _i, _j, _k, _l, _len, _m, _ref, _ref1, _ref2;
    ...
    for (_j = 0, _len = report.length; _j < _len; _j++) {
      ...
      for (recNum = _l = 0, _ref2 = sampleReport.records.length; 0 <= _ref2 ? _l < _ref2 : _l > _ref2; recNum = 0 <= _ref2 ? ++_l : --_l) {
        pos = columnOrder[recNum];
        rec = sampleReport.records[pos];
        metric = rec.metric;
    ...
    legend = '<span>';
    step = 6;
    for (hue = _m = RED_HUE; step > 0 ? _m <= GREEN_HUE : _m >= GREEN_HUE; hue = _m += step) {
  ...
  add_heatmap = function() {
    return $(".report_table td[number]").each(function() {
      var cell, k, max, maxHue, min, minHue, num, orther_numbers, other_cells, quality;
      ...
      if (((function() {
        var _i, _len, _results;
        _results = [];
        for (_i = 0, _len = orther_numbers.length; _i < _len; _i++) {
          num = orther_numbers[_i];
          if (num < 10) {
            _results.push(num);
          }
        }
      ...
      if (max === min) {
        return $(other_cells).css(css_property_to_color, get_color(GREEN_HUE));
      } else {
