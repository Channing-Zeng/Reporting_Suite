...
When samples are at risk of cross-species contamination (e.g. those derived from PDXs), then it is useful to assess the proportions of reads per sample that align to the different species involved. If the Disambiguate program was run to assess this, the next plot should show this information.


```{r Disambig, echo=FALSE, cache=TRUE, eval=doDisambig}
...
```{r GenLoc, echo=FALSE, cache=TRUE}
...
Depending on the library preparation methods used, the proportion of rRNA sequences should be quite low. If large proportions of rRNA sequences are seen, you may wish to consider if the
depth of the remaining sequences is sufficient for further analyses.

```{r rRNA, echo=FALSE, cache=TRUE}
...
```{r 5_3_bias, echo=FALSE, cache=TRUE}
...
```{r Corr_Heatmap, echo=FALSE, cache=TRUE}
...
Principal Components Analysis (PCA) is a very popular method that based on the principles of dimensional
