...
our ($opt_p, $opt_b, $opt_H, $opt_a, $opt_A, $opt_o, $opt_f, $opt_G);
getopts("Hp:b:a:A:o:f:G:") || USAGE();
$opt_H && USAGE();
...
while( <> ) {
    ...
    print "fi\n";
    print "cd $sample\n";
    print "qsub $option -cwd -V -N ${sample}_vardict -S /bin/bash $prog $bam $sample $bed $freq $genome $grch37\n";
    print "cd ..\n\n";
}
