...
# Usage: runVarDict_ion.sh 1.fastq sample target_bed freq
...
if [ ! -e ${SAMPLE}[._]sorted.realign.bam ]
    then
	/users/kdld047/local/bwa-0.7.4/bwa mem -t 8  -P -c 20 -R "@RG\tID:$SAMPLE\tSM:$SAMPLE" $REF/bwa/hg19.fa $FQ1 > $SAMPLE.sam 2> bwa.err 
...
if [ $BED ]
	...
	if [ ! -e done.vardict ]
	    then
	    for n in {1..8}; do
		minivardict_ion.sh $BAM ${SAMPLE} $BEDBASE $n $FREQ &
	    done
