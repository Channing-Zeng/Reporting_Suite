...
sample=$2
bed=$3
/users/kdld047/local/bwa-0.7.4/bwa mem -t 6 -P -c 20 -R "@RG\tID:$sample\tSM:$sample" /users/kdld047/work/NGS/NGS/genomes/hg19/Sequence/BWAIndex/genome.fa $fastq1 > ${sample}_hg19.sam 2> bwa_hg19.err
...
/users/kdld047/local/bwa-0.7.4/bwa mem -t 8 -P -c 20 -R "@RG\tID:$sample\tSM:$sample" /users/kdld047/work/NGS/NGS/genomes/mm10/Sequence/BWAIndex/genome.fa $fastq1 > ${sample}_mm10.sam 2> bwa_mm10.err
...
if [ $bed ]
	...
	/group/cancer_informatics/tools_resources/NGS/bin/checkSNV.pl -c 1 -s 2 -e 3 -S 2 -E 3 -g 4 -Q 10 -N $sample -b ${sample}_hg19_sorted.realign.disamb.bam $bed > ${sample}_alignment_summary.txt
