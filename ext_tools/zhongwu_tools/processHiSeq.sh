...
# Usage: processHiSeq.sh fastq_dir bed

#ls $1/*R[12]_*fastq.gz | pairFastq.pl -n "([^\/]*)_S\d+" > samples.txt # for MiSeq file names
...
for i in `getcols -c 2 samples.txt`; do countReads.pl -n "([^\/]*)_R[12].*" $i; done > read_counts.txt &

#sample2align.pl -b $2 samples.txt | addQueue2.pl > runAlign_hg19.sh
sample2align.pl -b $2 samples.txt > runAlign_hg19.sh
