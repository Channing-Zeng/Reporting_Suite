...
getopts('R:f:');
my $prefix = "";
$prefix = $opt_R ? "${opt_R}_" : $prefix;
$prefix = $opt_f ? "${opt_f}_$prefix" : $prefix; # such as Tumor
$opt_R = $opt_R ? qr/RG:Z:$opt_R/ : undef; # The read group
...
open( FQ1, ">${prefix}1.fastq");
open( FQ2, ">${prefix}2.fastq");
open( SIN, ">${prefix}singletons.fastq");
open( STAT, ">${prefix}SAM_stat.txt" );
