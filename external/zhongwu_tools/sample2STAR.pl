...
my $prog = $opt_p ? $opt_p : "/group/cancer_informatics/tools_resources/NGS/bin/runSTAR.sh";
while( <> ) {
    chomp;
    ...
    my $options = $opt_o ? $opt_o : "-l huge_ram=1 -l mem_free=32G";
    print "qsub $options -cwd -V -pe smp 15 -N ${sample}_STAR -S /bin/bash $prog $read1 $read2 $sample\n";
    print "cd ..\n\n";
}
