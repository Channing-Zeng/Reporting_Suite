...
my $option = $opt_o ? $opt_o : "-l huge_ram=1";

my $prog = $opt_p ? $opt_p : "/group/cancer_informatics/tools_resources/NGS/bin/vartect_hg19.sh";
...
while( <> ) {
    ...
    print "fi\n";
    print "cd $sample\n";
    print "qsub $option -cwd -V -N ${sample}_vardict -S /bin/bash $prog $bam $sample $bed\n";
    print "cd ..\n\n";
}
