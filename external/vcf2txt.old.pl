...
foreach my $d (@data) {
    ...
    $pass = "DUP" if ( $d->[24] && $d->[24] == 0 && $d->[22] !~ /1$/ && $d->[22] !~ /0$/ && (!$opt_a) && $d->[21] < 0.35 ); # all variants from one position in reads
    $pass = "MAXRATE" if ( $varn/$sam_n >= $MAXRATIO && $d->[21] < 0.35 ); # present in $MAXRATIO samples, regardless of frequency
    $pass = "QMEAN" if ($qmean && $qmean < $MINQMEAN );
    $pass = "PMEAN" if ($pmean && $pmean < $MINPMEAN );
    $pass = "MQ" if ( $d->[33] && $d->[33] < $MINMQ );
    $pass = "SN" if ( $d->[34] && $d->[34] < $SN );
    $pass = "MINFREQ" if ( $d->[21] && $d->[21] < $MINFREQ );
