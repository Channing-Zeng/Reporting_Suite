...
getopts('uHbR:F:f:n:r:p:q:c:D:P:Q:M:s:V:') || USAGE();
USAGE() if ( $opt_H );
...
my $MAF = 0.0025;
my $SN = $opt_s ? $opt_s : 1.5;
my $control = $opt_c;


# SBF: Strand Bias Fisher Exact test
my @columns = qw(CDS AA END DP AF BIAS PMEAN PSTD QUAL QSTD SBF GMAF VD CLNSIG ODDRATIO HIAF MQ SN ADJAF SHIFT3 MSI dbSNPBuildID TYPE STATUS SSF SOR M_DP M_VD M_AF);
print join("\t", qw(Sample Chr Start ID Ref Alt Type Effect Functional_Class Codon_Change Amino_Acid_Change Amino_Acid_Length Gene Transcript_bioType Gene_Coding Transcript Exon COSMIC_CDS_Change COSMIC_AA_Change End Depth AlleleFreq Bias Pmean Pstd Qual Qstd SBF GMAF VD CLNSIG ODDRATIO HIAF MQ SN AdjAF Shift3 MSI dbSNPBuildID Type Status Paired-p_value Paired-OddRatio Matched_Depth Matched_VD Matched_AlleleFreq N_samples N_Var Pcnt_sample Ave_AF PASS Var_Class)), "\n";
my @data;
my %sample;
my %var;
...
while( <> ) {
    ...
    my @fdata = split(/:/, $a[9]);
    my @mfdata = split(/:/, $a[10]);
    for(my $i = 0; $i < @formats; $i++) {
        $d{ $formats[$i] } = $fdata[$i];
    }
    for(my $i = 0; $i < @formats; $i++) {
        $d{ "M_$formats[$i]" } = $mfdata[$i];
    }
