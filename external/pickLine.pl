...
if ( $#ARGV < 0 ) {
    print "USAGE: $0 [-vuIC] [-d delimiter] [-p delimiter] [-i column] [-c column] [-D regexp] [-P regexp] [-l list] [-L char] [-k strings] [-K strings] ids_file files > outputfile\n";
    ...
    print "-i   Specify which column in ids_file contains the uniq id.\n";
    print "-c   Specify which column in files contains the uniq id.\n";
    print "-D   Regular expression to capture the ID in ids_fileas \$1.\n";
...
if ( $opt_i ) {
    @col1 = Util::Parser::indexes($opt_i);
}
if ( $opt_c ) {
    @col = Util::Parser::indexes($opt_c);
}
